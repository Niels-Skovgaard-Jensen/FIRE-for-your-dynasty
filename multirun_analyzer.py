...
class MultirunAnalyzer:
                ...
                if multirun_yaml.exists():
                    try:
                        ...
                        with open(multirun_yaml, 'r') as f:
                            ...
                            if 'overrides' in multirun_config and 'task' in multirun_config['overrides']:
                                data['sweep_overrides'] = multirun_config['overrides']['task']
                    except Exception:
    ...
    def create_comparison_dataframe(self) -> pl.DataFrame:
        ...
        for i, run in enumerate(self.runs_data):
            ...
            base_record = {
                ...
                'base_convergence_ratio': run['convergence_analysis']['convergence_ratio'],
    ...
    def generate_comparison_plots(self, output_dir: Path) -> None:
        ...
        if df_single.height > 0:
            ...
            scatter = plt.scatter(roi_data, investment_data, c=gap_data, s=size_data,
        ...
        if df_base.height > 0:
            ...
            plt.axhline(y=1, color='red', linestyle='--', alpha=0.5, label='Divergence Boundary')
        ...
        convergent_df = self.comparison_df.filter(
            (pl.col('scenario_converges') == True) & 
            (pl.col('scenario_cost').is_not_null())
        )
        ...
        if convergent_df.height > 0:
            for run_id in convergent_df['run_id'].unique():
                ...
                if run_data.height > 0:
                    ...
                    plt.plot(children_data, cost_data, 'o-', alpha=0.7, label=f'ROI: {roi:.1%}')
        ...
        if convergent_costs.height > 0:
            children_counts = sorted(convergent_costs['children_per_generation'].unique())
            cost_data = []
        ...
        plt.subplot(3, 3, 6)
        roi_analysis = self.comparison_df.filter(pl.col('children_per_generation') == 1).sort('roi_rate')
        ...
        if df_ratio.height > 0:
            ...
            scatter = plt.scatter(target_data, investment_ratio, c=roi_data, 
        ...
        plt.subplot(3, 3, 8)
        retirement_analysis = self.comparison_df.filter(pl.col('children_per_generation') == 1)
        if retirement_analysis.height > 0:
            ...
            scatter = plt.scatter(age_data, investment_data, c=roi_data, s=size_data,
        ...
        if self.comparison_df.height > 0:
            ...
            convergent = self.comparison_df.filter(pl.col('scenario_converges') == True)
            divergent = self.comparison_df.filter(pl.col('scenario_converges') == False)
            
            if convergent.height > 0:
                conv_roi = convergent['roi_rate'].to_numpy()
                conv_gap = convergent['generation_gap'].to_numpy()
                plt.scatter(conv_roi, conv_gap, c='green', alpha=0.6, label='Convergent', s=20)
                
            if divergent.height > 0:
                div_roi = divergent['roi_rate'].to_numpy()
                div_gap = divergent['generation_gap'].to_numpy()
                plt.scatter(div_roi, div_gap, c='red', alpha=0.6, label='Divergent', s=20)
    ...
    def generate_summary_report(self, output_dir: Path) -> None:
        ...
        with open(report_file, 'w') as f:
            ...
            f.write("-" * 20 + "\n")
            f.write(f"ROI Rates: {self.comparison_df['roi_rate'].min():.1%} - {self.comparison_df['roi_rate'].max():.1%}\n")
            f.write(f"Generation Gaps: {self.comparison_df['generation_gap'].min():.0f} - {self.comparison_df['generation_gap'].max():.0f} years\n")
            f.write(f"Target Amounts: {self.comparison_df['target_amount'].min():,.0f} - {self.comparison_df['target_amount'].max():,.0f}\n")
            f.write(f"Retirement Ages: {self.comparison_df['retirement_age'].min():.0f} - {self.comparison_df['retirement_age'].max():.0f} years\n\n")
            
            # Convergence Analysis
            f.write("Convergence Analysis\n")
            f.write("-" * 20 + "\n")
            
            convergence_summary = (self.comparison_df
                                 .group_by('children_per_generation')
                                 .agg([
            ...
            for row in convergence_summary.iter_rows(named=True):
                ...
                sum_val = row['sum']
                mean_val = row['mean']
                f.write(f"{children} children/generation: {sum_val}/{count} converge ({mean_val:.1%})\n")
            f.write("\n")
            ...
            single_child_df = self.comparison_df.filter(pl.col('children_per_generation') == 1)
            if single_child_df.height > 0:
                ...
                gap_corr = single_child_df.select([
                    pl.corr('generation_gap', 'single_child_investment').alias('gap_corr')
                ])['gap_corr'][0]
                ...
                target_corr = single_child_df.select([
                    pl.corr('target_amount', 'single_child_investment').alias('target_corr')
                ])['target_corr'][0]
                f.write(f"Target Amount vs Investment correlation: {target_corr:.3f}\n\n")
            ...
            if convergent_df.height > 0:
                for children in sorted(convergent_df['children_per_generation'].unique()):
                    subset = convergent_df.filter(pl.col('children_per_generation') == children)
                    if subset.height > 0:
                        best_idx = subset['scenario_cost'].arg_min()
                        best = subset.row(best_idx, named=True)
                        f.write(f"{children} children - Lowest cost: {best['scenario_cost']:,.0f}\n")
                        f.write(f"  ROI: {best['roi_rate']:.1%}, Gap: {best['generation_gap']:.0f}y, "
            ...
            for i, run in enumerate(self.runs_data):
                ...
                f.write(f"  Base convergence: {run['convergence_analysis']['converges']} ")
                f.write(f"(ratio: {run['convergence_analysis']['convergence_ratio']:.4f})\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze Dynasty FIRE multirun results")
    parser.add_argument("--outputs-dir", default="multirun", 
                       help="Directory containing Hydra multirun outputs (default: multirun)")
    parser.add_argument("--output-dir", default="multirun_analysis",
                       help="Directory to save analysis results (default: multirun_analysis)")
    parser.add_argument("--pattern", default="**/*/dynasty_results.json",
