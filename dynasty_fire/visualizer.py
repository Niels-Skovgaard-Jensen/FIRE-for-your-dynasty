...
class DynastyVisualizer:
    ...
    def create_dynasty_growth_plot(self, max_generations: Optional[int] = None) -> Dict[str, Any]:
        ...
        fig = plt.figure(figsize=[figsize[0], figsize[1] * 1.5])  # Make it taller for 6 subplots
    ...
    def _create_heatmap_subplot(self) -> None:
        ...
        im = plt.imshow(convergence_matrix, extent=[roi_range[0]*100, roi_range[-1]*100, 
                                                   children_range[0], children_range[-1]], 
                       aspect='auto', origin='lower', cmap='RdYlGn', alpha=0.8)
        
        # Add contour lines if enabled
        if self.config.visualization.add_contours:
            X, Y = np.meshgrid(roi_range * 100, children_range)
            contours = plt.contour(X, Y, convergence_ratio_matrix, levels=[1.0], colors='black', linewidths=2, alpha=0.8)
        
        # Add current configuration point
        plt.plot(self.dynasty.R * 100, self.config.analysis.children_per_generation, 
                'ro', markersize=8, label='Current Config')
    ...
    def _create_cost_comparison_subplot(self) -> None:
        ...
        for children in self.config.scenarios.children_options[:4]:  # Limit to 4 for readability
    ...
    def create_convergence_heatmap(self, roi_range: Optional[np.ndarray] = None, 
        ...
        if roi_range is None:
            roi_min, roi_max = self.config.scenarios.roi_range
            roi_range = np.linspace(roi_min, roi_max, self.config.visualization.heatmap_resolution)
        
        if children_range is None:
            children_range = np.linspace(1, 5, self.config.visualization.heatmap_resolution)
        ...
        if self.config.visualization.add_contours:
            ...
            contours = plt.contour(X, Y, convergence_ratio_matrix, levels=[1.0], colors='black', linewidths=3)
            ...
            ratio_contours = plt.contour(X, Y, convergence_ratio_matrix, levels=ratio_levels, 
                                       colors='gray', linewidths=1, linestyles='dashed', alpha=0.6)
