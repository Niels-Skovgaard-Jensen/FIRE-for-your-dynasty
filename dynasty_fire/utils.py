...
def save_results_to_json(results: Dict[str, Any], filename: str = "dynasty_results.json") -> Path:
...
def save_summary_report(results: Dict[str, Any], config: DictConfig, 
                       filename: str = "summary_report.txt") -> Path:
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"  Target Amount: {config.financial.target_amount:,.0f} {config.output.currency}\n")
        ...
        f.write(f"  Single Child Investment: {results['single_child_investment']:,.0f} {config.output.currency}\n")
        
        convergence = results['convergence_analysis']
        if convergence["infinite_sum"]:
            f.write(f"  Infinite Dynasty Cost: {convergence['infinite_sum']:,.0f} {config.output.currency}\n")
        ...
        for children, result in results['scenario_results'].items():
            ...
            if result['converges']:
                f.write(f"  {children_str} children per generation: {result['status']} - {result['cost']:,.0f} {config.output.currency}\n")
            else:
                f.write(f"  {children_str} children per generation: {result['status']} - ∞ (infinite)\n")
...
def format_currency(amount: float, currency: str = "DKK", decimal_places: int = 0) -> str:
